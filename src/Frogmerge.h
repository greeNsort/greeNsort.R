/*
# greeNsort common Frog merging
# Copyright (C) 2010-2024 Dr. Jens Oehlschaegel
# BSD-3 clause
# Provided 'as is', use at your own risk
*/

#ifndef ALREADY_DEFINED_Frogmerge_h
#define ALREADY_DEFINED_Frogmerge_h

#include "algo.h"

static void Frogmerge_asc_right_full(
  ValueT *x
...
)
{
  ...
  // for(int i=0;i<nl;i++)
  //   Rprintf("Frogmerge_asc_right_full ldat[%d]=%f\n", i, ldat[i]); R_FlushConsole();
  // for(int i=0;i<nr;i++)
  //   Rprintf("Frogmerge_asc_right_full rdat[%d]=%f\n", i, rdat[i]); R_FlushConsole();
...
static void Frogmerge_asc_right_final(
  ValueT *x
, ValueT *ldat  // pointer to left element of left stream
, IndexT nl        // number of values in left stream
, ValueT *rdat  // pointer to left element of right stream
...
static void Frogmerge_asc_right_stable(
  ValueT *x
, IndexT nl  // == b
, IndexT nr
, IndexT R
  ...
  // for(int i=ll;i<=lr;i++)
  //   Rprintf("Frogmerge_asc_right_stable left x[%d]=%f\n", i, x[i]); R_FlushConsole();
  // for(int i=rl;i<=rr;i++)
  //   Rprintf("Frogmerge_asc_right_stable right x[%d]=%f\n", i, x[i]); R_FlushConsole();
...
static void Frogmerge_asc_left_stable(
    ValueT *x
  , IndexT nl
  , IndexT nr  // == b
  , IndexT L
  ...
  // for(int i=ll;i<=lr;i++)
  //   Rprintf("Frogmerge_asc_left_stable left x[%d]=%f\n", i, x[i]); R_FlushConsole();
  // for(int i=rl;i<=rr;i++)
  //   Rprintf("Frogmerge_asc_left_stable right x[%d]=%f\n", i, x[i]); R_FlushConsole();
