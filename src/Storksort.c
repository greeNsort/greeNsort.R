...
static IndexT Storksort_initial_partition_Left_LE(
...
static void Storksort_partition_Left_LE(
    ValueT *t     // pointer to target
  , ValueT *s     // pointer to source
  , IndexT l      // leftmost position to be sorted
  , IndexT n      // number of elements
...
static void Storksort_partition_Left_LT(
    ValueT *t     // pointer to target
  , ValueT *s     // pointer to source
  , IndexT l      // leftmost position to be sorted
  , IndexT n      // number of elements
...
static void Storksort_partition_Left_GE(
    ValueT *t     // pointer to target
  , ValueT *s     // pointer to source
  , IndexT l      // leftmost position to be sorted
  , IndexT n      // number of elements
...
static void Storksort_partition_Left_GT(
    ValueT *t     // pointer to target
  , ValueT *s     // pointer to source
  , IndexT l      // leftmost position to be sorted
  , IndexT n      // number of elements
...
static void Storksort_partition_Right_LE(
    ValueT *t     // pointer to target
  , ValueT *s     // pointer to source
...
{
  IndexT l = r - n - n/2 + 1;  // leftmost buffer position
  IndexT i;
  n = r - n;
  for (i=r; i>n; i--){
    if (LE(s[i], v)){
...
static void Storksort_partition_Right_LT(
    ValueT *t     // pointer to target
  , ValueT *s     // pointer to source
  , IndexT r      // rightmost position to be sorted
  , IndexT n      // number of elements
...
static void Storksort_partition_Right_GE(
    ValueT *t     // pointer to target
  , ValueT *s     // pointer to source
  , IndexT r      // rightmost position to be sorted
  , IndexT n      // number of elements
...
static void Storksort_partition_Right_GT(
    ValueT *t     // pointer to target
  , ValueT *s     // pointer to source
  , IndexT r      // rightmost position to be sorted
  , IndexT n      // number of elements
...
static void Storksort_exsitu_rec_Left_LE(
...
static void Storksort_exsitu_rec_Right_GE(
  ValueT *x         // target data
, ValueT *y         // data + buffer
, IndexT ll         // leftmost target
...
static void Storksort_exsitu_rec_Right_LE(
  ValueT *x         // target data
, ValueT *y         // data + buffer
, IndexT ll         // leftmost target
, IndexT r          // rightmost value to be sorted
...
static void Storksort_exsitu_rec_Left_GE(
  ValueT *x         // target data
, ValueT *y         // data + buffer
, IndexT ll         // leftmost target
, IndexT l          // leftmost value to be sorted
...
, IndexT reversals  // number of direction reversals (needed to recover stable ties)
  ...
  //Rprintf("Storksort_exsitu_rec_Left_LE l=%d c0=%d v=%f c1=%d r=%d done=%d\n", l, c[0], v, c[1], r, done);
  if (done){
    x += ll;
    y += l;
    if (reversals%2){
    ...
    if (c[0] >= c[1]){
      ...
      Storksort_partition_Left_LE(y, y, l, n, v);  // reverses right wing
      if (c[0])
        Storksort_exsitu_rec_Left_GE(
          x          // target data
        , y          // data + buffer
        , ll         // leftmost target
      ...
      Storksort_partition_Left_GT(y, y, l, n, v);  // reverses right wing
      if (c[0])
        Storksort_exsitu_rec_Right_GE(
          x            // target data
        , y            // data + buffer
...
static void Storksort_exsitu_rec_Left_GE(
  ...
  //Rprintf("Storksort_exsitu_rec_Left_GE l=%d c0=%d v=%f c1=%d r=%d done=%d\n", l, c[0], v, c[1], r, done);
  if (done){
    x += ll;
    y += l;
    if (reversals%2){
    ...
    if (c[1] >= c[0]){
      ...
      Storksort_partition_Left_GE(y, y, l, n, v);  // reverses right wing
      if (c[0])
        Storksort_exsitu_rec_Right_LE(
          x            // target data
        , y            // data + buffer
        , ll           // leftmost target
        , r            // rightmost value to be sorted
      ...
      Storksort_partition_Left_LT(y, y, l, n, v);  // reverses right wing
      if (c[0])
        Storksort_exsitu_rec_Left_LE(
          x          // target data
        , y          // data + buffer
...
, IndexT reversals     // number of direction reversals (needed to recover stable ties)
  ...
  //Rprintf("Storksort_exsitu_rec_Right_GE l=%d c0=%d v=%f c1=%d r=%d done=%d\n", l, c[0], v, c[1], r, done);
  if (done){
    x += ll;
    y += (r - n + 1);
    if (reversals%2){
    ...
    if (c[1] >= c[0]){
      ...
      Storksort_partition_Right_GE(y, y, r, n, v);  // reverses left wing
      if (c[0])
        Storksort_exsitu_rec_Left_LE(
          x            // target data
        , y            // data + buffer
      ...
      Storksort_partition_Right_LT(y, y, r, n, v);  // reverses left wing
      if (c[0])
        Storksort_exsitu_rec_Right_LE(
          x            // target data
        , y            // data + buffer
...
static void Storksort_exsitu_rec_Right_LE(
  ...
  //Rprintf("Storksort_exsitu_rec_Right_LE l=%d c0=%d v=%f c1=%d r=%d done=%d\n", l, c[0], v, c[1], r, done);
  if (done){
    x += ll;
    y += (r - n + 1);
    if (reversals%2){
    ...
    if (c[0] >= c[1]){
      ...
      Storksort_partition_Right_LE(y, y, r, n, v);  // reverses left wing
      if (c[0])
        Storksort_exsitu_rec_Right_GE(
          x            // target data
        , y            // data + buffer
      ...
      Storksort_partition_Right_GT(y, y, r, n, v);  // reverses left wing
      if (c[0])
        Storksort_exsitu_rec_Left_GE(
          x          // target data
        , y          // data + buffer
...
static void Storksort_insitu_rec_Left_LE(
    ValueT *x         // target data
  , ValueT *y         // data + buffer
  , IndexT ll         // leftmost target
  , IndexT l          // leftmost value to be sorted
...
static void Storksort_insitu_rec_Right_GE(
    ValueT *x         // target data
  , ValueT *y         // data + buffer
  , IndexT ll         // leftmost target
  , IndexT r          // rightmost value to be sorted
...
static void Storksort_insitu_rec_Right_LE(
    ValueT *x         // target data
  , ValueT *y         // data + buffer
  , IndexT ll         // leftmost target
  , IndexT r          // rightmost value to be sorted
...
static void Storksort_insitu_rec_Left_GE(
    ValueT *x         // target data
  , ValueT *y         // data + buffer
  , IndexT ll         // leftmost target
  , IndexT l          // leftmost value to be sorted
...
static void Storksort_insitu_rec_Left_LE(
  ...
  //Rprintf("Storksort_insitu_rec_Left_LE l=%d c0=%d v=%f c1=%d r=%d done=%d\n", l, c[0], v, c[1], r, done);
  if (done){
    x += ll;
    y += l;
    if (reversals%2){
...
static void Storksort_insitu_rec_Left_GE(
  ...
  //Rprintf("Storksort_insitu_rec_Left_GE l=%d c0=%d v=%f c1=%d r=%d done=%d\n", l, c[0], v, c[1], r, done);
  if (done){
    x += ll;
    y += l;
    if (reversals%2){
...
static void Storksort_insitu_rec_Right_GE(
  ...
  //Rprintf("Storksort_insitu_rec_Right_GE l=%d c0=%d v=%f c1=%d r=%d done=%d\n", l, c[0], v, c[1], r, done);
  if (done){
    x += ll;
    y += (r - n + 1);
    if (reversals%2){
...
static void Storksort_insitu_rec_Right_LE(
  ...
  //Rprintf("Storksort_insitu_rec_Right_LE l=%d c0=%d v=%f c1=%d r=%d done=%d\n", l, c[0], v, c[1], r, done);
  if (done){
    x += ll;
    y += (r - n + 1);
    if (reversals%2){
...
, IndexT n
  ...
  if (n>1){
    ...
    // this is only a sensible method if the memory cost of x can be ignored
    ...
    /*done = */Storksort_initial_partition_Left_LE(
...
// Storksort cannot finalize insitu
// peudo insitu returns the sequence of partitioned values *before* finalization
// such that we can visualize this state
void Storksort_insitu(
    ValueT *x
  , IndexT n
){
  if (n>1){
