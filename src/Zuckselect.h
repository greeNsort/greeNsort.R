...
#ifndef ALREADY_DEFINED_Zuckselect_h
#define ALREADY_DEFINED_Zuckselect_h
...
static RangeIndexT Zuckselect_TieLeft(
    ValueT *x
  , IndexT l, IndexT r, IndexT k
);

// pivot partioning placing all pivot ties high
static RangeIndexT Zuckselect_TieRight(
    ValueT *x
  , IndexT l, IndexT r, IndexT k
){
  RangeIndexT ret;
  ...
  if(k<i){
    return Zuckselect_TieRight(x, l, i-1, k);
  }else{
    return Zuckselect_TieLeft(x, i, r, k);
  }
...
static RangeIndexT Zuckselect_TieLeft(
  ...
  if(k<=j){
    return Zuckselect_TieRight(x, l, j, k);
  }else{
    return Zuckselect_TieLeft(x, j+1, r, k);
  }
