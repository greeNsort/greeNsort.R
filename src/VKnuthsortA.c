/*
# greeNsort direct Knuthsort for variable-sized strings A-tuned
...
static void VKnuthsortAP_merge(char *z, char *l, char *m, char *r){
...
static void VKnuthsortA_merge(char *tar, char *src, IndexT l, IndexT m, IndexT r){
...
static void VKnuthsortA_merge_noregister(char *tar, char *src, IndexT l, IndexT m, IndexT r){
  IndexT i=l, j=strnext(src, m), k=l;
  IndexT s;
  for (;;){
    if (LT(src+j, src+i)){
...
static void VKnuthsortA_recurse(char *a, char *b, char *c, IndexT l, IndexT r){
  if (l<r){
    IndexT m,i;
#if VINSERTIONSORT_LIMIT > 0
    ...
    VKnuthsortA_recurse(b, a, c, l  , m);
    VKnuthsortA_recurse(b, a, c, strnext(a, m), r);
    // almost no difference between pointer and index implementation
    //VKnuthsortA_merge_noregister(a, b, l, m, r);
    //VKnuthsortA_merge(a, b, l, m, r);
    VKnuthsortAP_merge(a+l, b+l, b+m, b+r);
  }
}


void VKnuthsortA_insitu(
...
)
{
  ...
  VKnuthsortA_recurse(x, y, z, 1, m-1); // excluding leading and including trailing \0
  FREE(z);
  FREE(y);
  p->secs = getNewSecs();
...
)
{
  ...
  VKnuthsortA_recurse(y, y2, z, 1, m-1); // excluding leading and including trailing \0
  for (i=1;i<m;i++){
    x[i] = y[i];
  }
  FREE(z);
