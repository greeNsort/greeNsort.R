#ifndef ALREADY_DEFINED_SqrtsortP_h
#define ALREADY_DEFINED_SqrtsortP_h
...
/* size measurement with PerfT added                     */
...
static inline void sqrtsort_swap1(SORT_TYPE *a,SORT_TYPE *b){
	SORT_TYPE c=*a;
	*a++=*b;
	*b++=c;
}
static inline void sqrtsort_swapN(SORT_TYPE *a,SORT_TYPE *b,int n){
...
static void sqrtsort_MergeDown(SORT_TYPE *arr,SORT_TYPE *arr2,int L1,int L2){
	...
	while(p1<L2){
		if(p0==L1 || GE(arr+p0,arr2+p1)) arr[M++]=arr2[p1++];
		else arr[M++]=arr[p0++];
	}
	if(M!=p0) while(p0<L1) arr[M++]=arr[p0++];
}
...
static void sqrtsort_SortIns(SORT_TYPE *arr,int len){
	int i,j;
	for(i=1;i<len;i++){
		for(j=i-1;j>=0 && LT(arr+(j+1),arr+j);j--) sqrtsort_swap1(arr+j,arr+(j+1));
	}
}
...
// J.O. seemingly not used: 	if(M==0) nkeys=(M-1)/lblock+1;
	if(lrest<=LL){
		len-=lrest;
		lrest=0;
	}
...
void SqrtSort(SORT_TYPE *arr,int Len, PerfT *p){
	int L=1;
	SORT_TYPE *ExtBuf;
	int *Tags;
	...
	free(Tags);
	free(ExtBuf);
	p->size = 1 + (L*sizeof(SORT_TYPE) + NK*sizeof(int)) / (double) (Len*sizeof(SORT_TYPE));
}
