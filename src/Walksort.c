/*
# greeNsort Walksort (same code like Jumpsort)
...
#define Walksort_TRANSFER 0

static void Walksort_Mergesort_merge(ValueT *t, ValueT *s, IndexT l, IndexT m, IndexT r){
...
static void Walksort_Mergesort_recurse(ValueT *t, ValueT *s, IndexT l, IndexT r){
  if (l<r){
    IndexT m;
#if INSERTIONSORT_LIMIT > 0
...
#endif
    m = l + (r-l)/2;
    Walksort_Mergesort_recurse(s, t, l  , m);
    Walksort_Mergesort_recurse(s, t, m+1, r);
    Walksort_Mergesort_merge(t, s, l, m, r);
  }
}

static void Walksort_copyblock(ValueT *from, ValueT *to, IndexT b){
  IndexT i;
  if (from != to){
    for (i=0;i<b;i++)
      to[i] = from[i];
...
static void Walksort_copyright(
    ValueT **p  // pointer to pointers to blocks (data and buffer)
...
)
{
  ...
  if (t>0){
    ...
    for (i=l+2;i<=R;i++){
      if (os[i] <= l+1){
        o = os[ot[--t]];
        Walksort_copyblock(p[os[i]], p[o], b);
...
// finish with n data blocks right and 2 buffer blocks left
// moves from target back to source !!
static void Walksort_moveright(
    IndexT *os   // source (and final target) order of blocks (data and buffer)
  , IndexT *ot   // temporary tareget order of blocks (data and buffer)
  , IndexT l    // left data block (because of buffer left, add 2)
  , IndexT r   // rightmost of right data blocks (because of buffer left, add 2)
...
// finish with n data blocks left and 2 buffer blocks right
static void Walksort_mergeleft(
    ValueT **p  // pointer to pointers to blocks (data and buffer)
  , IndexT *os   // source (and final target) order of blocks (data and buffer)
...
{
  IndexT m;
  // Rprintf("Walksort_sortright l=%d r=%d b=%d\n", l, r, b);
  if (r <= l){
    ...
    Walksort_Mergesort_recurse(rp, lp, 0, b-1);  // sort to right
    return;
  }
  m = (l+r)/2;
  Walksort_sortright(p, o, o2, m+1, r, b);
  Walksort_sortright(p, o, o2, l, m, b);
  Walksort_mergeleft(p, o, o2, l, m, r, b);
#if Walksort_TRANSFER > 0
  Walksort_copyright(p, o, o2, l, r, b); // better for linear distance cost
#else
  Walksort_moveright(o, o2, l, r);       // better for constant or log distance cost
#endif
}

static void Walksort_Mergesort_insitu(ValueT *x, IndexT n)
{
  IndexT i;
  ...
  Walksort_Mergesort_recurse(x, buf, 0, n-1);
  FREE(buf);
}

static void Walksort_Mergesort_exsitu(ValueT *x, IndexT n)
{
  IndexT i;
  ...
  Walksort_Mergesort_recurse(buf, aux2, 0, n-1);
  for (i=0; i<n; i++)
    x[i] = buf[i];
  FREE(buf);
}
...
void Walksort_insitu(ValueT *x, IndexT n, IndexT b, PerfT *perf)
{
  // Special case: up to n == 4*b we switch to merge sort
  if (n <= 4*b){
    Walksort_Mergesort_insitu(x, n);
  ...
  k = K+r;
  m =  k / 2;
  // Rprintf("Walksort_exsitu n=%d b=%d K=%d r=%d R=%d\n", n, b, K, r, R);
  Walksort_sortright(p, o, o2, m , k-1, b);
  Walksort_sortright(p, o, o2, 0 , m-1, b);
  Walksort_mergeleft(p, o, o2, 0, m-1, k-1, b);
  Walksort_copyleft(p, o, o2, 0, k-1, b);
...
void Walksort_exsitu(ValueT *x, IndexT n, IndexT b, PerfT *perf)
{
  // Special case: up to n == 2*b we switch to merge sort
  if (n <= 2*b){
    Walksort_Mergesort_exsitu(x, n);
    perf->secs = getNewSecs();
    perf->size = 2;
    return;
  }
