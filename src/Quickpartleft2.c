...
static ValueT Quickpartleft2(ValueT *x, IndexT l, IndexT r, IndexT k)
{
#if INSERTIONSORT_LIMIT > 0
  ...
  SWAP(x[i], x[r], t);
  if (i == k){
    Quickpartleft2(x, l, i-1, k);
    return x[i];
  }else if(k<i){
    return Quickpartleft2(x, l, i-1, k);
  }else{
    Quickpartleft2(x, l, i-1, k);
    return Quickpartleft2(x, i+1, r, k);
  }
}

ValueT Quickpartleft2_insitu(ValueT *x, IndexT n, IndexT k){
  return Quickpartleft2(x, 0, n-1, k);
}
...
ValueT Quickpartleft2_exsitu(ValueT *x, IndexT n, IndexT k){
  ...
  for (i=0; i<n; i++)
    aux[i] = x[i];
  ret = Quickpartleft2(aux, 0, n-1, k);
  for (i=0;i<n;i++)
    x[i] = aux[i];
  FREE(aux);
  return ret;
