/*
# greeNsort direct Frogsort1 for variable-sized strings A-tuned
...
static IndexT VFrogsort1A_init(
    char *x
  , char *y
  , IndexT m
)
...
static void VFrogsort1A_sort_right(
    char *y
  , char *z
  , IndexT n
  , IndexT N
...
static void VFrogsort1A_sort_left(
    char *y
  , char *z
  , IndexT n
  , IndexT N
  ...
  if (n >= 1){
    ...
    if (n <= VINSERTIONSORT_LIMIT){
      //Rprintf("VFrogsort1A_sort_left VINSERTIONSORT_LIMIT n=%d ln=%d rn=%d l=%d r=%d N=%d LN=%d RN=%d L=%d LR=%d RL=%d R=%d\n", n, ln, rn, l, r, N, LN, RN, L, LR, RL, R);
    ...
    rn = n - ln;
    if (RN<=0){
      //Rprintf("VFrogsort1A_sort_left no feasible split n=%d ln=%d rn=%d N=%d LN=%d RN=%d L=%d LR=%d RL=%d R=%d\n", n, ln, rn, N, LN, RN, L, LR, RL, R);
      strdebuf_lr(y+L, y+L, y+R);
      VInsertionsort_l2r(y, z, L, L+n-1);
      return;
    }
    //Rprintf("VFrogsort1A_sort_left splitting n=%d ln=%d rn=%d N=%d LN=%d RN=%d L=%d LR=%d RL=%d R=%d\n", n, ln, rn, N, LN, RN, L, LR, RL, R);
    VFrogsort1A_sort_left (y, z, ln, LN, L);
    VFrogsort1A_sort_right(y, z, rn, RN, R);
    VFrogsort1AP_merge_asc_left_stable(y+L+n-1, y+L, y+L+ln-1, y+R-rn+1, y+R);
  }
}
...
)
{
  if (n >= 1){
...
#if VINSERTIONSORT_LIMIT > 0
    if (n <= VINSERTIONSORT_LIMIT){
      //Rprintf("VFrogsort1A_sort_right VINSERTIONSORT_LIMIT n=%d ln=%d rn=%d l=%d r=%d N=%d LN=%d RN=%d L=%d LR=%d RL=%d R=%d\n", n, ln, rn, l, r, N, LN, RN, L, LR, RL, R);
    ...
    ln = n - rn;
    //chrprint("VFrogsort1A_sort_right", y, L, R);
    if (LN<=0){
      //Rprintf("VFrogsort1A_sort_right no feasible split n=%d ln=%d rn=%d N=%d LN=%d RN=%d L=%d LR=%d RL=%d R=%d\n", n, ln, rn, N, LN, RN, L, LR, RL, R);
      strdebuf_rl(y+R, y+L, y+R);
      VInsertionsort_l2r(y, z, R-n+1, R);
      return;
    }
    //Rprintf("VFrogsort1A_sort_right splitting n=%d ln=%d rn=%d N=%d LN=%d RN=%d L=%d LR=%d RL=%d R=%d\n", n, ln, rn, N, LN, RN, L, LR, RL, R);
    VFrogsort1A_sort_left (y, z, ln, LN, L);
    VFrogsort1A_sort_right(y, z, rn, RN, R);
    VFrogsort1AP_merge_asc_right_stable(y+R-n+1, y+L, y+L+ln-1, y+R-rn+1, y+R);
  }
}


void VFrogsort1A_exsitu(
...
)
{
  ...
  aux[0] = NULL_CHAR;
  M = VFrogsort1A_init(x+1, aux+1, (m-1));
  if (M != maux)
    error("maux=%d M=%d\n", maux, M);
  VFrogsort1A_sort_left(aux, buf, (m-1), M, 1);
  for (i=1;i<m;i++){
    x[i] = aux[i];
  }
  FREE(buf);
...
)
{
  ...
  // ML = VFrogsort1A_init(x+1, aux+1, ml);
  // assert(ML == (ml+bl));
  VFrogsort1A_init(x+1, aux+1, ml);
  aux[0] = NULL_CHAR;
  //chrprint("left init", aux, 1, ML);
  // MR = VFrogsort1A_init(x+1+ml, x+m-mr-br, mr);
  // assert(MR == (mr+br));
  VFrogsort1A_init(x+1+ml, x+m-mr-br, mr);
  ...
  VFrogsort1A_sort_left (aux, buf, ml, ml+bl,   1);
  //chrprint("left sort", aux, 1, ML);
  VFrogsort1A_sort_right(  x, buf, mr, mr+br, m-1);
  //chrprint("rigt sort", x, 1, m-1);

  VFrogsort1AP_merge_asc_right_stable(x+1, aux+1, aux+ml, x+m-mr, x+m-1);

  //VFrogsort1AP_merge_asc_right_full(x+1, aux+1, aux+ML, x+m-MR+1, x+m);

  //VFrogsort1A_initsort_left (x+1     , aux+1              , buf, ml, 0     , 0          );
  //VFrogsort1A_initsort_right(x+1 + ml, x+1 + (m - mr - br), buf, mr, mr - 1, mr + br - 1);
  // VFrogsort1A_init_left (x+1     , aux+1              , buf, ml, 0     , 0          );
  // VFrogsort1A_init_right(x+1 + ml, x+1 + (m - mr - br), buf, mr, mr - 1, mr + br - 1);
  // VFrogsort1A_sort_left (aux+1              , buf, ml, 0     );
  // VFrogsort1A_sort_right(x+1 + (m - mr - br), buf, mr, mr + br - 1);
  //VFrogsort1A_merge_asc_right_final(x+1, aux+1, ml, x+1 + (m - mr), mr);
  FREE(buf);
  FREE(aux);
  p->secs = getNewSecs();
