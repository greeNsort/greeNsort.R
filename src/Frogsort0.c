...
static void Frogsort0_init(ValueT *from, ValueT *to, IndexT n){
  IndexT i, ntri = n / 2;
  if (n % 2){
    to[0] = R_NegInf;
    to[2] = from[0];
...
static void Frogsort0_merge_right_final(
  ValueT *x
...
static void Frogsort0_merge_left(
...
static void Frogsort0_merge_right(
  ValueT *x // pointer to triplets
, IndexT l     // leftmost triplet
, IndexT m     // number of triplets in left stream, we must have m <= n-m
, IndexT n     // total number of triplets
...
static void Frogsort0_TunedCase_left(
...
static void Frogsort0_TunedCase_right(
  ValueT *x // pointer to working memory
, IndexT l     // leftmost triplet to sort
, IndexT n     // number of triplets to sort
){
...
static void Frogsort0_BaseCase_left(
...
static void Frogsort0_BaseCase_right(
  ValueT *x // pointer to triplet
){
  if (LT(x[2], x[0])){
    x[1] = x[2];
...
static void Frogsort0_sort_right(
  ValueT *x // pointer to triplets
, IndexT l     // leftmost triplet to sort
, IndexT n     // number of triplets to sort
);
static void Frogsort0_sort_left(
  ValueT *x // pointer to triplets
, IndexT l     // leftmost triplet to sort
, IndexT n     // number of triplets to sort
){
#if INSERTIONSORT_LIMIT > 0
  if (n <= TRIPLET_LIMIT){
    Frogsort0_TunedCase_left(x,l,n);
  }
#else
  if (n==1){
    Frogsort0_BaseCase_left(x+3*l);
  }
#endif
  else if(n>1){
    IndexT m = n - n/2; // Frogsort0_merge_left must have m >= n-m
    Frogsort0_sort_left (x, l  ,   m);
    Frogsort0_sort_right(x, l+m, n-m);
    Frogsort0_merge_left(x, l, m, n);
  }
}
static void Frogsort0_sort_right(
  ValueT *x // pointer to triplets
...
#if INSERTIONSORT_LIMIT > 0
  if (n <= TRIPLET_LIMIT){
    Frogsort0_TunedCase_right(x,l,n);
  }
#else
  if (n==1){
    Frogsort0_BaseCase_right(x+3*l);
  }
#endif
  else if(n>1){
    IndexT m = n/2; // Frogsort0_merge_right must have m <= n-m
    Frogsort0_sort_right(x, l+m, n-m);
    Frogsort0_sort_left (x, l  ,   m);
    Frogsort0_merge_right(x, l, m, n);
  }
}
...
void Frogsort0_insitu(ValueT *x, IndexT n)
{
  ...
  Frogsort0_init(x, aux, nl);
  Frogsort0_init(x+nl, ori, nr);
  Frogsort0_sort_right(ori, 0, rtri);
  Frogsort0_sort_left(aux, 0, ltri);
  Frogsort0_merge_right_final(x, aux+nl%2, nl, ori+rtri, nr);
  FREE(aux);
  return;
}
...
void Frogsort0_exsitu(ValueT *x, IndexT n)
{
  ...
  Frogsort0_init(x, aux, n);
  Frogsort0_sort_left(aux, 0, ntri);
  if (n%2)
    aux++;
  for (IndexT i=0; i<n; i++)
    x[i] = aux[i];
