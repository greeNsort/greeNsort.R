/*
# greeNsort Katasort (pointer implementation)
...
static void KatasortP_merge(ValueT *z, ValueT *l, ValueT *m, ValueT *r){
  ValueT*j=m+1;
  ValueT u=*l, v=*j;
  if (LT(*r, *m)){
...
static void KatasortP_recurse(ValueT *a, ValueT *b, IndexT n){
  if (n>1){
    IndexT m;
#if INSERTIONSORT_LIMIT > 0
...
#endif
    m = n/2;
    KatasortP_recurse(b, a, m);
    KatasortP_recurse(b+m, a+m, n-m);
    KatasortP_merge(a, b, b+m-1, b+n-1);
  }
}
