#ifndef ALREADY_DEFINED_GrailsortP_h
#define ALREADY_DEFINED_GrailsortP_h
...
/*                                                       */
/* (c) 2016 by Jens Oehlschlägel                         */
/*                                                       */
/* most calls to SORT_CMP have been replace by called to */
/* one of LT LE EQ NE GE GT and those are implemented as */
/* macros (inline), not as C-functions                   */
/* replaced <inline>  by <static inline>  for -O0        */
/*********************************************************/
...
static void grail_MergeWithoutBuffer(SORT_TYPE *arr,int len1,int len2){
	...
	if(len1<len2){
		...
		while(len2){
			...
			do{
				len2--;
			} while(len2 && LE(arr+len1-1,arr+len1+len2-1));
		}
	}
}
...
static void grail_MergeLeftWithXBuf(SORT_TYPE *arr,int L1,int L2,int M){
	int p0=0,p1=L1; L2+=L1;
	while(p1<L2){
		if(p0==L1 || GT(arr+p0,arr+p1)) arr[M++]=arr[p1++];
		else arr[M++]=arr[p0++];
	}
...
// J.O. seemingly not used: int plast;
	...
	lrest=lblock;
	frest=LT(keys,midkey) ? 0 : 1;
	pidx=lblock;
	for(cidx=1;cidx<nblock;cidx++,pidx+=lblock){
		prest=pidx-lrest;
		fnext=LT(keys+cidx,midkey) ? 0 : 1;
		if(fnext==frest){
	...
	prest=pidx-lrest;
	if(llast){
// J.O. seemingly not used: plast=pidx+lblock*nblock2;
		if(frest){
			memcpy(arr+prest-lblock,arr+prest,lrest*sizeof(SORT_TYPE));
			prest=pidx;
...
static void grail_SortIns(SORT_TYPE *arr,int len){
	int i,j;
	for(i=1;i<len;i++){
		for(j=i-1;j>=0 && LT(arr+(j+1),arr+j);j--) grail_swap1(arr+j,arr+(j+1));
	}
}

static void grail_LazyStableSort(SORT_TYPE *arr,int L){
	int m,h,p0,p1,rest;
// J.O. seemingly not used: int u;
	for(m=1;m<L;m+=2){
// J.O. seemingly not used: u=0;
		if(GT(arr+m-1,arr+m)) grail_swap1(arr+(m-1),arr+m);
	}
	for(h=2;h<L;h*=2){
		p0=0;
		p1=L-2*h;
...
// J.O. seemingly not used: int nkeys;
	SORT_TYPE *arr1;
	
	M=len/(2*LL);
	lrest=len%(2*LL);
// J.O. seemingly not used: nkeys=(2*LL)/lblock;
	if(lrest<=LL){
		len-=lrest;
		lrest=0;
	}
	...
	for(b=0;b<=M;b++){
		...
		for(u=1;u<NBlk;u++){
			...
			for(v=u;v<NBlk;v++){
				...
				if(kc>0 || (kc==0 && GT(keys+p,keys+v))) p=v;
			}
			if(p!=u-1){
		...
		if(llast!=0){
			while(nbl2<NBlk && LT(arr1+NBlk*lblock,arr1+(NBlk-nbl2-1)*lblock)) nbl2++;
		}
...
static void grail_RecMerge(SORT_TYPE *A,int L1,int L2){
	...
	if(k2<L1 && EQ(A+k2,A+K)) k2=grail_BinSearchRight(A+k1,L1-k1,A+K)+k1;
	m1=grail_BinSearchLeft(A+L1,L2,A+K);
	m2=m1;
	if(m2<L2 && EQ(A+L1+m2,A+K)) m2=grail_BinSearchRight(A+L1+m1,L2-m1,A+K)+m1;
