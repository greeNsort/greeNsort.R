...
static void Zackpart_TieRight(
  ...
  if (i < p[0]){
    Zackpart_TieLeft(x, i+1, r, p);
  }else if(i > p[1]){
    Zackpart_TieLeft(x, l, i-1, p);
  }else{
    Zackpart_TieLeft(x, l, i-1, p);
    Zackpart_TieLeft(x, i+1, r, p);
  }
...
static void Zackpart_TieLeft(
  ...
  if (j < p[0]){
    Zackpart_TieRight(x, j+1, r, p);
  }else if(j > p[1]){
    Zackpart_TieRight(x, l, j-1, p);
  }else{
    Zackpart_TieRight(x, l, j-1, p);
    Zackpart_TieRight(x, j+1, r, p);
  }
...
void Zackpart_insitu(ValueT *x, IndexT n, IndexT *partial){
  Zackpart_TieLeft(x, 0, n-1, partial);
  return;
}
...
void Zackpart_exsitu(ValueT *x, IndexT n, IndexT *partial){
  ...
  for (i=0; i<n; i++)
    aux[i] = x[i];
  Zackpart_TieLeft(aux, 0, n-1, partial);
  for (i=0; i<n; i++)
    x[i] = aux[i];
  FREE(aux);
  return;
