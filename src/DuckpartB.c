/*
# greeNsort Duck partial sorting algorithm B-tuned
...
static void DuckpartB_TieRight(
  ...
  if (i < p[0]){
    DuckpartB_TieLeft(x, i+1, r, p);
  }else if(i > p[1]){
    DuckpartB_TieRight(x, l, i-1, p);
  }else{
    DuckpartB_TieRight(x, l, i-1, p);
    DuckpartB_TieLeft(x, i+1, r, p);
  }
...
static void DuckpartB_TieLeft(
  ...
  if (j < p[0]){
    DuckpartB_TieLeft(x, j+1, r, p);
  }else if(j > p[1]){
    DuckpartB_TieRight(x, l, j-1, p);
  }else{
    DuckpartB_TieRight(x, l, j-1, p);
    DuckpartB_TieLeft(x, j+1, r, p);
  }
...
void DuckpartB_insitu(ValueT *x, IndexT n, IndexT *partial){
  DuckpartB_TieLeft(x, 0, n-1, partial);
  return;
}
...
void DuckpartB_exsitu(ValueT *x, IndexT n, IndexT *partial){
  ...
  for (i=0; i<n; i++)
    aux[i] = x[i];
  DuckpartB_TieLeft(aux, 0, n-1, partial);
  for (i=0;i<n;i++)
    x[i] = aux[i];
  FREE(aux);
  return;
