...
static RangeIndexT Zackpartright_TieLeft(
    ValueT *x
...
// pivot partioning placing all pivot ties high
static RangeIndexT Zackpartright_TieRight(
    ValueT *x
  , IndexT l, IndexT r, IndexT k
){
  RangeIndexT ret;
  ...
  if(k<i){
    //Zackpartright_TieLeft(x, i, r, k);
    Zacksort_TieLeft(x, i, r);
    return Zackpartright_TieLeft(x, l, i-1, k);
  }else{
    return Zackpartright_TieLeft(x, i, r, k);
  }
...
static RangeIndexT Zackpartright_TieLeft(
  ...
  if(k<=j){
    //Zackpartright_TieRight(x, j+1, r, k);
    Zacksort_TieRight(x, j+1, r);
    return Zackpartright_TieRight(x, l, j, k);
  }else{
    return Zackpartright_TieRight(x, j+1, r, k);
  }
...
RangeIndexT Zackpartright_insitu(ValueT *x, IndexT n, IndexT k){
  return Zackpartright_TieLeft(x, 0, n-1, k);
}
...
RangeIndexT Zackpartright_exsitu(ValueT *x, IndexT n, IndexT k){
  ...
  for (i=0; i<n; i++)
    aux[i] = x[i];
  ret = Zackpartright_TieLeft(aux, 0, n-1, k);
  for (i=0;i<n;i++)
    x[i] = aux[i];
  FREE(aux);
  return ret;
