...
output: 
  rmarkdown::html_vignette:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{greeNsort-1-quickstart}
...
## Introducing greeNsort

The *greeNsort* R package is the development- and test-bed for the sustainable sorting algorithms.
It provides convenient R APIs for generating various validation datasets, for running and measuring various sorting algorithms (implemented in C) and for generic RAPL measurements.
The focus is on measuring Energy and Footprint KPIs and fair comparison of algorithms implemented in equal quality, not on providing algorithms for R. 
Most implementations focus sorting doubles, some handle strings as an example of sorting size-varying elements. 
The package is a moving target as it evolves. This *greeNsort-1-quickstart*  vignette explains the basic usage of the package. After this you might want to read the [*greeNsort-measurement*](greeNsort-measurement.html) vignette and for testing and development the [*greeNsort-2-development*](greeNsort-2-development.html) vignette.


## Installing greeNsort

See the README.md for explanations
- how to install all required linux libraries including R
- how to install all required R libraries
- how to install greeNsort (and greeNsort.Rcpp)

Note that you are not ready to perform a package check or use the package before you went through the following configuration steps:

## Configuring greeNsort

Running a *greeNsort<sup>&reg;</sup>* algorithm in the package automatically measures it and returns RAM-usage, runTime, and RAPL energy measurements. This requires appropriate configuration and compilation for your machine, and appropriate access grants to the RAPL counters.

Currently the package supports Intel and AMD processors und Linux on one-socket machines. 

### Configured Compilation

The *greeNsort* R package directly reads from the powercap RAPL counters using simple custom code. 
Once you have installed the package and loaded it with 
...
If it returns `TRUE` everthing is fine. If it returns `FALSE` you need to configure your `src/powercap_config.h` by calling 

```
perfconfig()
```

and recompile the package `greeNsort` (and also `greeNsort.Rcpp`).


### Granting RAPL Access

There are several ways to grant access to the RAPL counters to your user. If you know the root password, a convenient way to grant access is calling

```
perfgrants()
```

and enter the root root password for the embedded `sudo` call. Type `perfgrants` without parenthesis to see the function definition: it's pure interpreter code and you can easily verify what it is doing with the root access. You will need to call `perfgrants()` after each reboot of the machine. If you don't have the root password or want a more permanent solution talk to your system administrator and show him the `chmod` calls that `perfgrants()` outputs if you don't enter a password.


## Using the sorting API

The standard `sort()` function in R does not change its input and returns a sorted vector in newly allocated memory. Type `?runif` and `?sort` to see the online help of those R functions.
...
The package algorithms do manipulate (i.e. change) their input in original memory and return measurement results:
...
To leave the input untouched you must deepcopy it: 
...
Usage of measurement API is convenient. For details read the [*greeNsort-measurement*](greeNsort-measurement.html) vignette. 
We now create a bigger dataset where sorting costs enough work that you can measure it with a single measurement, we use a parallel random number generator:
...
Usually you are not interested in the raw measurement components returned, rather in the KPI evaluation of those using `perf()`:

```{r}
perf(k)
```

We focus here on a small subset of these KPIs that you get with `sperf()`
...
---- | ---------- | -------
%RAM |  (data + buffer) / data memory | a fixed cost measure of embedded hardware invest needed
runTime | elapsed time in seconds | a variable cost measure of operational cost
tFootprint | runTime * %RAM | a combined measure of variable and fixed cost
bcdEnergy | RAPL Energy measured at the CPU socket during sorting in Joules | a variable cost measure of operational cost
bcdFootprint | bcdEnergy * %RAM | a combined measure of variable and fixed cost

`runTime` is the classic measure of algorithmic performance, regarding CO2 cost `bcdEnergy` is more relevant. Since algorithms have a memory-performance trade-off, the Footprint measures allow a fair comparison of algorithms that have different memory needs. Let's compare a couple of algorithms, ordered ascending by `bcdFootprint`:
...
*Knuthsort* is a optimal classic Mergesort using 100% buffer space. It serves as a reference for other stable sorting algorithms. Hence let's normalize all KPIs for *Knuthsort*. To do so we use the matrix transpose function `t()`, because R treats matrices column-wise:

```{r}
t(k)
```

For the normalization we transpose, divide by *Knuthsort*, transpose back and order decreasing by `bcdFootprint`:
...
We can also have this graphically
...
*IMergesort* is a classic Inplace-Mergesort using 0% buffer. It is a clear looser on all KPIs. *Grailsort* is an excellent Inplace-Mergesort by Huang&Langston (1989-1992) implemented by Andrey Astrelin (2013). Due to the inevitable memory-performance trade-off, *Grailsort* cost twice the runTime and Energy of *Knuthsort*, but fair comparison with Footprints shows that *Grailsort* can compete with *Knuthsort*. *Frogsort1* using 50% buffer is sligtly slower and costs slightly more Energy than *Knuthsort*, but fair comparison with Footprints shows that *Frogsort1* is more sustainable than *Knuthsort*. Finally, *Frogsort2* is a clear winner on all KPIs, with about 3/4 of the operational costs and about 40% of the Footprint. Most remarkable is, that *Frogsort2* has lower operational cost, despite using less memory compared to *Knuthsort*.

We can also add and subtract using `perfplus` and `perfminus`:
...
So far we have used the standard random number generator `runif()` or its parallel cousin `prunif()`. The greeNsort package comes with a rich database of data generator functions that are organized in a `?data.frame`, see `?testdb`. Let's check the row- and column-names: 

```{r}
dimnames(testdb)
```

`testdb` has `label`s in `rownames()` and certain features in `colnames()`. Let's look at descriptions and interpreter code of the functions:
...
Let's pick out those ones, that we have used for the total evaluation in the *greeNsort<sup>&reg;</sup>* innovation report:
...
To get randomly permuted numbers from 1 to `N` you can

```{r}
N <- 7
testdb$func$permut(N)
```
We can also do this progamatically, a picture says more than thousand words:
...
```
We see easily that `permut` is completely unsorted (and has no duplicates), `tielog` has a logarithmic number of unique values and hence a lot of duplicates, `ascall` is perfectly presorted, `asclocal` has `sqrt(N)` locally ascending sequences (no duplicates, the type of data *Timsort* likes) and `ascglobal` has globally ascending clusters, each locally unsorted (no duplicates). 



## Working with algorithms

The algorithms in the greeNsort package are organized in a `?data.frame`, see `?algodb`. Each algorithms is described in a row across several feature columns:

```{r}
dimnames(algodb)
```

There are algorithms for different tasks, and some of them are stable (keep the original order in case of tied keys):
...
The package as an (approximate) MEDIAN algorithm, some SELECTion  algorithms, many SORTing algorithms. The lowercase 'sort' algorithms are only suitable for very small datasets because they scale $\BigO{N^2}$; they are used for the recursion leaves of the SORT algorithms, and are made available in R only for testing purposes. 
...
Let's check for an optimal buffer fraction for *Frogsort2*. 
...
Adaptivity for presorted data is coded in `algodb$adapt_asc`:

```{r}
table(algodb$adapt_asc, useNA="ifany")
```

Most sorting algorithms (*soft*) behave somewhat adaptive although this is not designed into the algorithm, some algorithms are automatically adaptive by design (*hard*), some algorithms are *tuned* to be adaptive accepting some trade-off (often denoted `*sortA`), and few are not adaptive at all (*none*). `NA` represents algorithms where this attribute is unknown or inappropriate. Let's look at the *hard* and *tuned* ones:
...
Here are some stored measurements for a subset of those (AMD® Ryzen 7 pro 6850, 8 physical cores):
...
This was measured with the following code, try it out on your machine:
...
There are two totals, `ASC` is the average cost over the `permut` and `asc*` datasets, `ALL` is the average cost over the `permut` (twice) and `asc*` and `desc*` datasets. By convention sorting is rather ascending, hence we consider `ASC` practically more relevant, `ALL` is included to provide a benchmark for bi-adaptive algorithms (*Octo*, *Gecko1*, *Squid1*, *Squid2*, *Tim*, *Peek*, *Power*). While *Timsort*, *Peeksort* and *Powersort* excel at presorted data, they pay a tuning price on randomly permuted data. In a mixed portfolio of tasks such as `ASC` they play in the same league like a simple *Knuthsort*. The simple un-tuned *Frog2* algorithm is a clear speed-winner. The parallel `PFrog1` achieved a speedup of 6x with permuted data, very little speedup with perfectly presorted data, and 5x for `ASC`. `Frogsort2` loses its speed advantage over `Frogsort1` when parallelized. Note that *Timsort* and *Powersort* are inherently serial.
...
Regarding *eFootprints* the differences are more pronounced,  *Timsort*, *Peeksort* and *Powersort* win over *Knuthsort*, but not over the *greeNsort<sup>&reg;</sup>* algorithms. *Frogsort2* emerges as a clear winner, particularly if parallelized. 
...
Parallel sorting can save energy, on single user machines because the CPU can quicker return to lower energy states.
...
The data was create with this code:
...
Median and Selection algorithms can be used to calculate the 50% percentile, so let's look at them together. The greeNsort package has

```{r}
algodb[algodb$task %in% c("MEDIAN", "SELECT"),c("name","best","average","worst")]
```

The median of the sequence `1:100` is calculated in R as `median(1:100)` = `r median(1:100)`, which in the even case is the average of the two mid-most values `mean(50:51)`. A generalization is available as `quantile(1:100, probs=c(0.5, 0.6))` = `r quantile(1:100, probs=c(0.5, 0.6))`. We can force to return an existing value by `quantile(1:100, probs=0.5, type=1)` = `r quantile(1:100, probs=0.5, type=1)` which is biased downwards and give the value at position at position 50, not 51 (R counts from 1, unlike C which counts from 0). 

Selection algorithms typically return the value at a specified position in a hypothetically sorted set without incurring the cost of full sorting. Take the following example
...
The 50th value, at position 50 is `r y[50]`. A famous deterministic algorithm for finding this at lower cost is *BFPRT*, that of Blum, Floyd, Pratt, Rivest & Tarjan (1973), which uses a *Median of Medians* sub-algorithm.
...
It returns `r y[50]` together with an attribute `attr(, "perf")` holding the measurement. We can seperate the two by 
...
*greeNsort<sup>&reg;</sup>* discovered that *BFPRT* does unnecessary work, which can be avoided by using the *Medians of Medians* sub-algorithm to find an *approximate median* and plugging it as pivot in a classic *Quickselect*, see below. Here we go:

```{r}
as.vector(approxMedian(as.double(1:100)))
```

This version does not change its input and needs only 20% buffer:

```{r}
as.vector(approxMedian(as.double(1:100), situation = "exsitu"))
```

And here is *Pickselect* which uses `approxMedian`:

```{r}
as.vector(Pickselect(as.double(1:100)))
```

The symmetric *greeNsort<sup>&reg;</sup>* version *Zickselect* is more informative: at the same costs it properly handles ties and returns the leftmost and rightmost position of elements tied with the specified position 
...
To get the value itself take the k-th element from `y`:

```{r}
y[k[1]]
```
This works because Zickselect's input has been properly *partially* sorted:

```{r}
y
```

For a visualization of the difference to *BFPRTselect* and *Pickselect* see
...
It is known that the classic probabilistic *Quickselect* is faster than the deterministic *BFPRT*. The symmetric *Zackselect* improves up this with proper tie handling and the more informative return values. *ZuckselectB* is a version tuned to avoid branch-misprediction. 

Let's measure R's `median` and `quantile` functions:
...
Now let's measure some algorithms of the greeNosrt package. approxMedian with `situation = "exsitu"` protects its input, all other calls don't:
...
Let's take the names from the measurement, and show the results without the measurement:
...
Some of the functions return the median `r m`, however the `Z*select` functions return more informative the range of the positions that are tied to the median. This can easily verified by 

```{r}
c(diff(l[["Zickselect"]])+1, sum(x==m))
```

Let's measure allocating plus copying to put the results better in context:
...
Of the package algorithms, the classic `BFPRTselect` is most expensive, `Pickselect` (and `Zickselect`) saves about `r round(100*(1-r["Pickselect","bcdEnergy"]/r["BFPRTselect","bcdEnergy"]))`%, `Quickselect` (and `Zackselect`) saves about `r round(100*(1-r["Quickselect","bcdEnergy"]/r["BFPRTselect","bcdEnergy"]))`%, and `ZuckselectB` saves about `r round(100*(1-r["ZuckselectB (1x)","bcdEnergy"]/r["BFPRTselect","bcdEnergy"]))`%. 

For fair comparison we need to substract `alloc.copy` from `median` and `quantile`. The median can be replaced by a call to `ZuckselectB` in the odd case (saves `r round(100*(1-r["ZuckselectB (1x)","bcdEnergy"]/(r["median","bcdEnergy"]-r["alloc.copy","bcdEnergy"])))`%), and by a call to `ZuckpartB`^[Note that the `Z*part` and `Z*partB` functions do not return the tie-extended range of the sorted interval, but can be modified at minor cost to do so] in the even case (saves `r round(100*(1-r["ZuckpartB","bcdEnergy"]/(r["median","bcdEnergy"]-r["alloc.copy","bcdEnergy"])))`%), and even calling `ZuckselectB` twice is cheaper (saves `r round(100*(1-r["ZuckselectB (2x)","bcdEnergy"]/(r["median","bcdEnergy"]-r["alloc.copy","bcdEnergy"])))`%). 

That's quite a savings potential.


### Partial Sorting

There are two types of partial sort definitions generalizing *Selection*. 

- requiring multiple element that should be in the correct position in a partially sorted set (R's `quantile`)
- requiring a sorted interval in the correct position in a partially sorted set (C++'s ,  greeNsort's `*part*` functions)

The function `Quickpartleft` is similar to the partial sorting of C++, it sorts the lowest `k` elements. The more general `Quickpart` sorts between position `k[1]` and `k[2]`. With `k[1]==1` this does the same as `Quickpartleft`. The symmetric greeNsort functions `Z*ckpart*` and `D*ckpart*` benefit from their proper tie handling. `Zackpartleft` returns the leftmost and rightmost position tied with `k`. `Zackpart` returns only measurements (but could easily be modified to return four positions).
...
Let's now compare four algorithms: `Dotnetpart` is a (suboptimal) real-world implementation, `Quickpart` is an efficient prior-art algorithm, `Zuckpart` is a new symmetric algorithm, and `ZuckpartB` is a version tuned to avoid branch-misprediction. 
...
The data generators of `testdb` can be used to generate string data:

```{r}
num2str(testdb$func$stable(7))
```

These are suitable for creating string data with similar characteristics as the numeric data, but these are not natural strings. The *greeNsort<sup>&reg;</sup>* package has `vtestdb` with elements of natural language,  for example variants of Kafka's "Die Verwandlung":
...
Finally note that *VFrogsort* uses a reserved rare character `BUFFER_CHAR` to mark empty buffers. The R-function `string_check()` returns the first occurence of `BUFFER_CHAR` in its input vector of strings (and 0 for strings that do not contain `BUFFER_CHAR`, hopefully)
...
The Kafka datasets are small. The bible has a larger corpus of words:
...
Let's compare three algorithms for size-varying elements:
...
*WQuicksort2* is an indirekt *Quicksort2* sorting and stabilizing pointers to strings. *UZacksort* is an indirect *Zacksort* which does not stablize and hence exploits duplicates. *VKnuthsort* is a *Knuthsort* that sorts (and moves) strings directly. *VFrogsort1* is a *Frogsort1* that sorts (and moves) strings directly.

Given the huge number of duplicated words in the bible, *UZacksort* appears as a clear winner here, and the stabilized Quicksort is a clear loser. However, note that the *Footprints* of *VFrogsort1* can compete with *UZacksort*: *VFrogsort1* has the lowest `%RAM` because elements are small and the Quicksorts need extra memory for pointers. 

Now let's turn to longer sequences of words, 5-grams:
...
5-grams have very few duplicates. Let's sort these:
...
Now the Quicksorts have the lowest `%RAM`, but *VKnuthsort* wins on `runTime` and `bcdEnergy` and *VFrogsort1* wins on the Footprints.

Finally let's look at the dictionary data:
...
Dictionary words have no duplicates and are (almost) sorted.
...
Due to its adaptiveness, *VFrogsort1* almost reaches the performance of *VKnuthsort* with lower memory demands.


## Sorting matrix data

So far we only sorted keys. What happens if we sort records with a key? What happens if we vary the size of the payload? We implemented some algorithms to sort 32bit-integer matrix-columns by keys in the first matrix row, so all rows after the first row contain payload data. The following algorithms are available for matrix data:
...
The algorithms with an initial 'M' in the name move columns consisting of key and payload during the $\BigO{N\log{N}}$ algorithm. The algorithms with an initial 'M' in the name separate the keys, sort the keys $\BigO{N\log{N}}$ and finally permute the columns $\BigO{N}$. 
...
Note that *Frogsort1* is a special case of  *Frogsort2* with `p=0.5`. Let's look at runTime normalized by *Knuthsort* at randomly permuted data:
